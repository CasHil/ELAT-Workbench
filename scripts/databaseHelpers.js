  ...
  connection
    ...
    .then(function (isExist) {
      if (isExist) {
        ...
        toastr.info(
          "Welcome! If this is your first time here, visit <a href='https://mvallet91.github.io/ELAT'>ELAT Home</a> to learn more",
          "ELAT",
          { timeOut: 7000 },
        );
        let dbQuery = getEdxDbQuery();
        connection.runSql(dbQuery).then(function (tables) {
          toastr.success("Database generated, please reload the page", "ELAT", {
            timeOut: 5000,
          });
          console.log(tables);
          loader(false);
...
export function sqlInsert(table, dataRows, connection) {
  ...
  let info = "";
  if (table === "webdata" || table === "metadata") {
  ...
  connection
    ...
    .then(function (rowsAdded) {
      if (rowsAdded > 0 && table !== "forum_interaction") {
        ...
        if (table === "metadata") {
          loader(false);
          toastr.success("Please reload the page now", "Metadata ready", {
            timeOut: 0,
          });
        }
      }
...
export function populateSamples(courseId, connection) {
  loader(true);
  let courseMap = {
    FP101x: "DelftX+FP101x+3T2015.json",
  ...
  connection.runSql("SELECT * FROM webdata").then(function (metadata) {
    if (metadata.length > 1) {
      ...
      $.getJSON(courseFile, function (json) {
        ...
        toastr.success("Please reload the page now", "Sample data ready", {
          timeOut: 0,
        });
        loader(false);
      });
  ...
  if (!testing) {
    ...
    connection.runSql(query).then(function (result) {
      ...
      for (let segment of segmentMap[segmentation]) {
        connection.runSql(
          "DELETE FROM webdata WHERE name = 'graphElements_" + segment + "'",
        );
        connection.runSql(
          "DELETE FROM webdata WHERE name = 'databaseDetails_" + segment + "'",
        );
        connection.runSql(
          "DELETE FROM webdata WHERE name = 'mainIndicators_" + segment + "'",
        );
        connection.runSql(
          "DELETE FROM webdata WHERE name = 'arcElements_" + segment + "'",
        );
        connection.runSql(
          "DELETE FROM webdata WHERE name = 'cycleElements_" + segment + "'",
        );
        connection.runSql(
...
export function clearDashboardTablesWebdata(connection) {
  ...
  connection.runSql(query).then(function (result) {
    if (result.length === 1) {
      ...
      for (let segment of segmentMap[segmentation]) {
        connection.runSql(
          "DELETE FROM webdata WHERE name = 'courseDetails_" + segment + "'",
        );
        connection.runSql(
          "DELETE FROM webdata WHERE name = 'mainIndicators_" + segment + "'",
        );
        connection
          .runSql(
            "DELETE FROM webdata WHERE name = 'databaseDetails_" +
              segment +
              "'",
          )
          .then(function () {
            ...
            toastr.success(
              ...
              "Updating Indicators and Charts",
              { timeOut: 0 },
            );
          });
      }
  ...
  if (r === false) {
    ...
    await connection
      ...
      .catch(function (err) {
        console.log(err);
        alert(
          "The deletion process started but did not finish,\n please refresh and try again",
        );
      });
    await connection
      .clear("video_interactions")
      .then(function () {
