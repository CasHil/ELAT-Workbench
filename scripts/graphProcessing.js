...
function drawAreaChart(graphElementMap, startDate, endDate, weekly) {
  ...
  let weeklyDurations = groupWeeklyMapped(
      graphElementMap,
      "orderedAvgDurations",
    ),
    weeklyVideoSessions = groupWeeklyMapped(
      graphElementMap,
      "orderedVideoSessions",
    ),
    weeklyQuizSessions = groupWeeklyMapped(
      graphElementMap,
      "orderedQuizSessions",
    ),
    weeklyForumSessions = groupWeeklyMapped(
      graphElementMap,
      "orderedForumSessions",
    );
 ...
 */
function calculateDropoutValues(
  courseMetadataMap,
  lastSessions,
  lastElements,
  connection,
  ...
  elements.sort(function (a, b) {
    return dropoutFreq[b] - dropoutFreq[a];
  });
      ...
      for (let fullElement in courseMetadataMap.child_parent_map) {
        if (fullElement.includes(element)) {
          ...
          if (elementId.split("_").length > 1) {
            elementName = elementName + "_" + elementId.split("_")[1];
          }
          topElements[elementId] = {
  ...
  for (let topElement in slicedTopElements) {
    ...
    for (let dateString of dateList) {
      ...
      if (dateString in orderedDropoutsByElement[topElement]) {
        orderedDropoutsByElementValue[topElement][date] =
          orderedDropoutsByElement[topElement][dateString].length;
      } else {
        orderedDropoutsByElementValue[topElement][date] = 0;
      }
    }
    orderedDropoutsByElementWeeklyValue[topElement] = groupWeekly(
      orderedDropoutsByElementValue[topElement],
    );
  }
  ...
  connection
    .runSql(
      "DELETE FROM webdata WHERE name = 'dropoutElements_" + segment + "'",
    )
...
function drawAreaDropoutChart(startDate, endDate, connection, weekly, segment) {
  ...
  areaDropoutCtx.clearRect(
    0,
    0,
    areaDropoutCanvas.width,
    areaDropoutCanvas.height,
  );
  ...
  connection
    .runSql(
      "SELECT * FROM webdata WHERE name = 'dropoutElements_" + segment + "' ",
    )
    .then(function (result) {
      if (result.length !== 1) {
        ...
        let dropoutValues = result[0]["object"],
          orderedDropoutsByElementValue =
            dropoutValues.orderedDropoutsByElementValue,
          orderedDropoutsByElementWeeklyValue =
            dropoutValues.orderedDropoutsByElementWeeklyValue,
        ...
        for (let element in orderedDropoutsByElementValue) {
          ...
          if (weekly === true) {
            ...
            dateLabels = Object.keys(
              orderedDropoutsByElementWeeklyValue[element],
            );
          } else {
...
function drawBoxChart(graphElementMap, startDate, endDate, weekly) {
  ...
  let weeklyPostContents = groupWeeklyMapped(
    graphElementMap,
    "orderedForumPostContent",
  );
  let weeklyRegPostContents = groupWeeklyMapped(
    graphElementMap,
    "orderedForumPostContentRegulars",
  );
  let weeklyOccPostContents = groupWeeklyMapped(
    graphElementMap,
    "orderedForumPostContentOccasionals",
  );
  ...
  if (weekly === true) {
    ...
    regPostContentData = Object.values(
      graphElementMap["orderedForumPostContentRegulars"],
    );
    occPostContentData = Object.values(
      graphElementMap["orderedForumPostContentOccasionals"],
    );
  }
    ...
    datasets: [
      {
        label:
          "Posts by Regulars - " +
          graphElementMap["forumSegmentation"]["regularPosters"] +
          " students",
      ...
      {
        label:
          "Posts by Occasionals - " +
          graphElementMap["forumSegmentation"]["occasionalPosters"] +
          " students",
...
function drawMixedChart(graphElementMap, startDate, endDate, weekly) {
  ...
  let weeklyRegPosts = groupWeeklyMapped(
    graphElementMap,
    "orderedForumPostsByRegulars",
  );
  let weeklyRegPosters = groupWeeklyMapped(
    graphElementMap,
    "orderedForumPostersRegulars",
  );
  let weeklyOccPosts = groupWeeklyMapped(
    graphElementMap,
    "orderedForumPostsByOccasionals",
  );
  let weeklyOccPosters = groupWeeklyMapped(
    graphElementMap,
    "orderedForumPostersOccasionals",
  );
  let weeklyForumSessions = groupWeeklyMapped(
    graphElementMap,
    "orderedForumAvgDurations",
  );
  let weeklyForumStudents = groupWeeklyMapped(
    graphElementMap,
    "orderedForumStudents",
  );
  let weeklyForumRegulars = groupWeeklyMapped(
    graphElementMap,
    "orderedForumRegulars",
  );
  let weeklyForumOccasionals = groupWeeklyMapped(
    graphElementMap,
    "orderedForumOccasionals",
  );
  ...
  if (weekly === true) {
    ...
    forumRegPosters = trimByDates(
      weeklyRegPosters["weeklySum"],
      startDate,
      endDate,
    );
    forumOccData = trimByDates(weeklyOccPosts["weeklySum"], startDate, endDate);
    forumOccPosters = trimByDates(
      weeklyOccPosters["weeklySum"],
      startDate,
      endDate,
    );
    ...
    forumStudentsRegulars = trimByDates(
      weeklyForumRegulars["weeklySum"],
      startDate,
      endDate,
    );
    forumStudentsOccasionals = trimByDates(
      weeklyForumOccasionals["weeklySum"],
      startDate,
      endDate,
    );
  } else {
    ...
    forumRegData = trimByDates(
      graphElementMap["orderedForumPostsByRegulars"],
      startDate,
      endDate,
    );
    forumRegPosters = trimByDates(
      graphElementMap["orderedForumPostersRegulars"],
      startDate,
      endDate,
    );
    forumOccData = trimByDates(
      graphElementMap["orderedForumPostsByOccasionals"],
      startDate,
      endDate,
    );
    forumOccPosters = trimByDates(
      graphElementMap["orderedForumPostersOccasionals"],
      startDate,
      endDate,
    );
    ...
    forumStudentsRegulars = trimByDates(
      graphElementMap["orderedForumRegulars"],
      startDate,
      endDate,
    );
    forumStudentsOccasionals = trimByDates(
      graphElementMap["orderedForumOccasionals"],
      startDate,
      endDate,
    );
  }
...
function drawHeatChart(graphElementMap) {
      ...
      formatter: function (value) {
        for (let group in graphElementMap["gradeLists"]) {
          ...
          if (gradeAvg(grades) === value) {
            return (
              value + JSON.stringify(graphElementMap["counterLists"][group])
            );
          } else {
            return value * 100;
          }
        ...
        data: [
          {
            x: "Regular Viewers",
            y: zeroIfEmptyArray(
              graphElementMap["gradeLists"]["nonPostersRegViewers"],
            ),
          },
          {
            x: "Occasional Viewers",
            y: zeroIfEmptyArray(
              graphElementMap["gradeLists"]["nonPostersOccViewers"],
            ),
          },
          {
            x: "Non-Viewers",
            y: zeroIfEmptyArray(
              graphElementMap["gradeLists"]["nonPostersNonViewers"],
            ),
          },
        ],
      },
        ...
        data: [
          {
            x: "Regular Viewers",
            y: zeroIfEmptyArray(
              graphElementMap["gradeLists"]["occPostersRegViewers"],
            ),
          },
          {
            x: "Occasional Viewers",
            y: zeroIfEmptyArray(
              graphElementMap["gradeLists"]["occPostersOccViewers"],
            ),
          },
          {
            x: "Non-Viewers",
            y: zeroIfEmptyArray(
              graphElementMap["gradeLists"]["occPostersNonViewers"],
            ),
          },
        ],
      },
        ...
        data: [
          {
            x: "Regular Viewers",
            y: zeroIfEmptyArray(
              graphElementMap["gradeLists"]["regPostersRegViewers"],
            ),
          },
          {
            x: "Occasional Viewers",
            y: zeroIfEmptyArray(
              graphElementMap["gradeLists"]["regPostersOccViewers"],
            ),
          },
          {
            x: "Non-Viewers",
            y: zeroIfEmptyArray(
              graphElementMap["gradeLists"]["regPostersNonViewers"],
            ),
          },
        ],
      },
  ...
  heatChart = new ApexCharts(document.querySelector("#heatChart"), heatOptions);
  heatChart.render();
}
        ...
        for (let segment of segmentMap[segmentation]) {
          ...
          for (let learner of learnerIds) {
            ...
            if (segment === "none" || seg === segment) {
              ...
              for (let session of learnerSessions) {
                if (
                  session.video_id !== currentVideo &&
                  session.video_id in videoIds
                ) {
          ...
          for (let video in videoIds) {
            ...
            for (let nextVideo in videoIds) {
              if (frequency.hasOwnProperty(nextVideo)) {
                frequency[nextVideo] =
                  frequency[nextVideo] / (failingViewers + passingViewers); // For normalized value
              }
            }
          ...
          for (let currentVideo in frequencies) {
            let percentages =
              ...
              course_metadata_map["element_name_map"][
                "block-v1:" + courseId + "+type@video+block@" + currentVideo
              ] +
              "</span><br>";
          ...
          await connection.runSql(
            "DELETE FROM webdata WHERE name = 'arcElements_" + segment + "' ",
          );
 ...
 * @param {string} segment Current segment to evaluate
 */
function drawVideoTransitionArcChart(connection, segment) {
  // https://www.d3-graph-gallery.com/graph/arc_template.html
  if (!segment) {
    segment = "none";
  }
  connection
    .runSql("SELECT * FROM webdata WHERE name = 'arcElements_" + segment + "' ")
    .then(function (result) {
      if (result.length !== 1) {
        ...
        let x = d3.scalePoint().range([0, width]).domain(allNodes);
        ...
        let nodes = svg
          .selectAll("mynodes")
          .data(
            nodeData.nodes.sort(function (a, b) {
              return +b.n - +a.n;
            }),
          )
          .enter()
          ...
          .on("mouseover", function (d) {
            ...
            labels
              ...
              .style("font-size", function (label_d) {
                ...
                if (
                  label_d.name === d.name ||
                  nodeTargetMap[d.id].includes(label_d.id)
                ) {
                  return 12;
                } else {
                  return 2;
        ...
        for (let segment of segmentMap[segmentation]) {
          ...
          for (let currentElement in frequenciesDesigned) {
            for (let followingElement in frequenciesDesigned[currentElement]) {
              ...
              const sourceType =
                  course_metadata_map.element_type_map[currentElement],
                targetType =
                  course_metadata_map.element_type_map[followingElement],
          ...
          for (let learnerId of learnerIds) {
              ...
              .then(function (sessions) {
                ...
                sessions.forEach(function (session) {
                  ...
                  if (elementId.length < 10) {
                    ...
                    forumStartSession["elementId"] = elementId.slice(
                  ...
                  if (elementId.length < 10) {
                    ...
                    forumEndSession["elementId"] = elementId.slice(
                      elementId.lastIndexOf("@") + 1,
                    );
                  }
            ...
            await connection
              ...
              .then(function (sessions) {
                ...
                sessions.forEach(function (session) {
                  ...
                  quizStartSession["elementId"] = elementId.slice(
                    elementId.lastIndexOf("@") + 1,
                    elementId.indexOf("_course"),
                  );
                  ...
                  quizEndSession["elementId"] = elementId.slice(
                    elementId.lastIndexOf("@") + 1,
                    elementId.indexOf("_course"),
                  );
            ...
            await connection
              ...
              .then(function (sessions) {
                ...
                sessions.forEach(function (session) {
                  ...
                  session["elementId"] = elementId.slice(
                    elementId.lastIndexOf("@") + 1,
                  );
                  session["status"] = status;
                  allSessions[learnerId].push(session);
            ...
            if (allSessions[learnerId].length > 2) {
              lastElements.push(
                allSessions[learnerId][allSessions[learnerId].length - 1][
                  "elementId"
                ],
              );
              lastSessions.push(
          ...
          do {
            ...
            for (let learnerId in allSessions) {
              ...
              for (let session of allSessions[learnerId]) {
                if (
                  new Date(session.time) > weekStart &&
              ...
              if (learningPath.length > 1) {
                learningPaths[learnerStatus[learnerId]][learnerId] =
                  learningPath;
              }
            }
            ...
            for (let status in learningPaths) {
              ...
              for (let learnerId in learningPaths[status]) {
                for (
                  let i = 0;
                  i < learningPaths[status][learnerId].length - 1;
                  i++
                ) {
                  let currentElement = learningPaths[status][learnerId][i];
                  let followingElement =
                    learningPaths[status][learnerId][i + 1];
            ...
            for (let status in elementIds) {
              ...
              for (let element in elementIds[status]) {
                ...
                for (let nextElement in elementIds[status]) {
                  if (frequency.hasOwnProperty(nextElement)) {
                    frequency[nextElement] =
                      frequency[nextElement] /
                      elementIds[status][element].length;
                  }
                }
            ...
            weekStart < new Date(course_metadata_map.end_date.toDateString())
          );
          ...
          for (let week in weeklyData) {
            ...
            for (let status in frequencies) {
              for (let currentElement in frequencies[status]) {
                for (let followingElement in frequencies[status][
                  currentElement
                ]) {
                  let sourceType = currentElement.slice(
                    0,
                    currentElement.indexOf("_"),
                  );
                  let targetType = followingElement.slice(
                    0,
                    followingElement.indexOf("_"),
                  );
                  ...
                  let link = {
                    ...
                    targetNode: targetNode,
                    value:
                      frequencies[status][currentElement][followingElement],
                    status: status,
                    id: " " + l,
                  };
                  links.push(link);
          ...
          let cycleElements = [
            { name: "cycleElements_" + segment, object: cycleData },
          ];
          console.log("Deleting from storage at " + new Date());
          await connection.runSql(
            "DELETE FROM webdata WHERE name = 'cycleElements" + segment + "'",
          );
  ...
  connection
    ...
    .then(function (result) {
      ...
      if (result.length !== 1) {
        ...
        link
          .attr("d", function (d) {
            // https://stackoverflow.com/a/17687907/8331561
            i++;
          ...
          .attr("stroke", function (d) {
            if (d.status === "designed" && cycleType === "designed") {
              ...
              (d.status !== "downloadable" && cycleType === "notpassing") ||
              (d.status !== "downloadable" && cycleType === "general")
            ) {
              return "red";
            } else if (
              (d.status === "downloadable" && cycleType === "downloadable") ||
              (d.status === "downloadable" && cycleType === "general")
            ) {
              return "green";
