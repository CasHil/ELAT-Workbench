...
export async function getGraphElementMap(connection, segment) {
  ...
  let promise = new Promise((resolve, reject) => {
    ...
    connection
      .runSql(
        "SELECT * FROM webdata WHERE name = 'graphElements_" + segment + "' ",
      )
                ...
                connection
                  ...
                  .then(function (courses) {
                    courses.forEach(async function (course) {
                      ...
                      let query =
                        "SELECT * FROM webdata WHERE name = 'segmentation' ";
                      let segmentation = "";
                      ...
                      for (let segment of segmentMap[segmentation]) {
                            ...
                            sessions.forEach(function (session) {
                              ...
                              if (segment === "none" || seg === segment) {
                                ...
                                if (dailyDurations.hasOwnProperty(start)) {
                                  dailyDurations[start].push(
                                    session["duration"],
                                  );
                                  dailySessions[start].push(
                                    session["course_learner_id"],
                                  );
                                } else {
                            ...
                            q_sessions.forEach(function (session) {
                              ...
                              if (segment === "none" || seg === segment) {
                                ...
                                if (quizDurations.hasOwnProperty(start)) {
                                  quizDurations[start].push(
                                    session["duration"],
                                  );
                                  quizSessions[start].push(
                                    session["course_learner_id"],
                                  );
                                } else {
                                  quizDurations[start] = [];
                            ...
                            v_sessions.forEach(function (session) {
                              ...
                              if (segment === "none" || seg === segment) {
                                ...
                                if (videoDurations.hasOwnProperty(start)) {
                                  videoDurations[start].push(
                                    session["duration"],
                                  );
                                  videoSessions[start].push(
                                    session["course_learner_id"],
                                  );
                                } else {
                        ...
                        await connection
                          ...
                          .then(function (questions) {
                            questions.forEach(function (question) {
                              dueDates.push(
                                question["question_due"].toDateString(),
                              );
                            });
                          });

                        let quizDates = Array.from(new Set(dueDates));
                        let annotationsDue = quizDates.map(
                          function (date, index) {
                            return {
                              type: "line",
                              id: "line" + index,
                        ...
                        for (let el in course_metadata_map.element_time_map) {
                          if (el.includes("problem")) {
                            startDates.push(
                              course_metadata_map.element_time_map[
                                el
                              ].toDateString(),
                            );
                          }
                        ...
                        let annotationStart = quizReleaseDates.map(
                          function (date, index) {
                            return {
                              type: "line",
                        ...
                        let annotations =
                          annotationsDue.concat(annotationStart);

                        query = "SELECT * FROM forum_sessions";
                        await connection
                          .runSql(query)
                          .then(function (f_sessions) {
                            f_sessions.forEach(function (session) {
                              ...
                              if (segment === "none" || seg === segment) {
                                ...
                                if (forumDurations.hasOwnProperty(start)) {
                                  forumDurations[start].push(
                                    session["duration"],
                                  );
                                  forumSessions[start].push(
                                    session["course_learner_id"],
                                  );
                                } else {
                        ...
                        await connection
                          .runSql(query)
                          .then(function (f_interactions) {
                            f_interactions.forEach(function (interaction) {
                              const seg = learnerSegmentation(
                                interaction["course_learner_id"],
                                segmentation,
                                connection,
                              );
                              if (seg === null) {
                              ...
                              if (segment === "none" || seg === segment) {
                                let timestamp =
                                  interaction["post_timestamp"].toDateString();
                                timestamp = new Date(timestamp);
                                posterPostMap[interaction["post_content"]] =
                                  interaction["course_learner_id"];
                                if (forumPosts.hasOwnProperty(timestamp)) {
                                  forumPosts[timestamp].push(
                                    interaction["post_content"],
                                  );
                                  forumPosters[timestamp].push(
                                    interaction["course_learner_id"],
                                  );
                                } else {
                        ...
                        const forumSegmentation = await getForumSegmentation(
                          weeklyPosters,
                          weeklyFViewers,
                          connection,
                        );
                        ...
                        let regularPosters = forumSegmentation.regularPosters,
                          ...
                          occasionalPosters =
                            forumSegmentation.occasionalPosters,
                          occasionalViewers =
                            forumSegmentation.occasionalViewers;
                        ...
                        let regPostersRegViewers = intersection(
                            new Set(regularPosters),
                            new Set(regularViewers),
                          ),
                          regPostersOccViewers = intersection(
                            new Set(regularPosters),
                            new Set(occasionalViewers),
                          ),
                          regPostersNonViewers = difference(
                            new Set(regularPosters),
                            new Set([...regularViewers, ...occasionalViewers]),
                          ),
                          occPostersRegViewers = intersection(
                            new Set(occasionalPosters),
                            new Set(regularViewers),
                          ),
                          occPostersOccViewers = intersection(
                            new Set(occasionalPosters),
                            new Set(occasionalViewers),
                          ),
                          occPostersNonViewers = difference(
                            new Set(occasionalPosters),
                            new Set([...regularViewers, ...occasionalViewers]),
                          ),
                          nonPostersRegViewers = difference(
                            new Set([...regularPosters, ...occasionalPosters]),
                            new Set(regularViewers),
                          ),
                          nonPostersOccViewers = difference(
                            new Set([...regularPosters, ...occasionalPosters]),
                            new Set(occasionalViewers),
                          ),
                          nonPostersNonViewers = difference(
                            idSet,
                            new Set([
                              ...regularPosters,
                              ...occasionalPosters,
                        ...
                        for (let group in forumMatrixGroups) {
                          ...
                          for (let student of forumMatrixGroups[group]) {
                            if (gradeMap.hasOwnProperty(student)) {
                              if (
                                gradeMap[student]["certificate_status"] ===
                                "downloadable"
                              ) {
                                gradeLists[group].push(
                                  gradeMap[student]["final_grade"],
                                );
                                passing++;
                              } else {
                                notPassing++;
                        ...
                        for (let date of dateList) {
                          ...
                          orderedStudents[date] = new Set(
                            dailySessions[date],
                          ).size;
                          orderedDurations[date] = dailyDurations[date];

                          orderedForumStudents[date] = new Set(
                            forumSessions[date],
                          ).size;

                          if (quizSessions.hasOwnProperty(date)) {
                            orderedQuizSessions[date] =
                              quizSessions[date].length;
                          } else {
                            orderedQuizSessions[date] = 0;
                          }

                          if (videoSessions.hasOwnProperty(date)) {
                            orderedVideoSessions[date] =
                              videoSessions[date].length;
                          } else {
                          ...
                          if (forumSessions.hasOwnProperty(date)) {
                            orderedForumSessions[date] =
                              forumSessions[date].length;
                            ...
                            orderedForumSessionOccasionals[date] =
                              occasionals.length;
                            orderedForumOccasionals[date] = new Set(
                              occasionals,
                            ).size;
                          } else {
                          ...
                          if (forumPosters.hasOwnProperty(date)) {
                            orderedForumPosters[date] = Math.round(
                              forumPosters[date].length,
                            );
                            for (let poster of forumPosters[date]) {
                              if (regularPosters.includes(poster)) {
                                orderedForumPostsByRegulars[date] =
                                  orderedForumPostsByRegulars[date] + 1;
                                orderedForumPostersRegulars[date].push(poster);
                              } else {
                                orderedForumPostsByOccasionals[date] =
                                  orderedForumPostsByOccasionals[date] + 1;
                                orderedForumPostersOccasionals[date].push(
                                  poster,
                                );
                              }
                            }
                          ...
                          orderedForumPostersRegulars[date] = new Set(
                            orderedForumPostersRegulars[date],
                          ).size;
                          orderedForumPostersOccasionals[date] = new Set(
                            orderedForumPostersOccasionals[date],
                          ).size;

                          if (forumPosts.hasOwnProperty(date)) {
                            orderedForumPosts[date] = Math.round(
                              forumPosts[date].length,
                            );
                            let dailyContent = [];
                            let dailyContentReg = [];
                            let dailyContentOcc = [];
                            for (let post of forumPosts[date]) {
                              ...
                              if (
                                regularPosters.includes(posterPostMap[post])
                              ) {
                          ...
                          for (
                            let i = 0;
                            i < dailyDurations[date].length;
                            i++
                          ) {
                            total += dailyDurations[date][i];
                          }
                          orderedAvgDurations[date] = parseFloat(
                            total / dailyDurations[date].length,
                          ).toFixed(2);

                          let quizTotal = 0;
                          for (
                            let i = 0;
                            i < orderedQuizSessions[date].length;
                            i++
                          ) {
                            quizTotal += quizDurations[date][i];
                          }
                          orderedQuizDurations[date] =
                            quizTotal / orderedQuizSessions[date];

                          let vidTotal = 0;
                          for (
                            let i = 0;
                            i < orderedVideoSessions[date].length;
                            i++
                          ) {
                            vidTotal += videoDurations[date][i];
                          }
                          orderedVideoDurations[date] =
                            vidTotal / orderedVideoSessions[date].length;

                          let forumTotal = 0;
                          if (forumDurations.hasOwnProperty(date)) {
                            for (
                              let i = 0;
                              i < forumDurations[date].length;
                              i++
                            ) {
                              forumTotal += forumDurations[date][i];
                            }
                            orderedForumDurations[date] =
                              Math.round(forumTotal);
                            orderedForumAvgDurations[date] = Math.round(
                              forumTotal / forumDurations[date].length,
                            );
                          } else {
                        ...
                        graphElementMap = {
                          ...
                          orderedForumSessionRegulars:
                            orderedForumSessionRegulars,
                          orderedForumSessionOccasionals:
                            orderedForumSessionOccasionals,
...
export function exportChartPNG(chartId) {
  ...
  if (element.className === "chartjs-render-monitor") {
    ...
    SVG2PNG(element.firstElementChild, function (canvas) {
 ...
 * @param {JsStoreWorker} connection Main JsStore worker that handles the connection to SqlWeb
 */
export async function getForumSegmentation(
  weeklyPosters,
  weeklyViewers,
  connection,
) {
...
export function groupWeeklyMapped(graphElementMap, orderedSeriesName) {
  let grouped = _.groupBy(graphElementMap["dateListChart"], (result) =>
    moment(result, "DD/MM/YYYY").startOf("isoWeek"),
  );
  let weeklySum = {};
  let weeklyAvg = {};
  let weekType = "number";
...
export function groupWeekly(elementObject) {
  let grouped = _.groupBy(Object.keys(elementObject), (result) =>
    moment(new Date(result), "DD/MM/YYYY").startOf("isoWeek"),
  );
  let weeklySum = {};
  for (let week in grouped) {
    let weekDays = grouped[week];
