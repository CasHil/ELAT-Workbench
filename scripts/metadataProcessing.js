 ...
 */
export async function processMetadataFiles(files, connection) {
  loader(true);
  ...
  if (Object.keys(courseMetadataMap).length < 1) {
    ...
    if (
      ...
      if (learnerDemographicRecord.length === 0) {
        ...
        for (let learner of learnerDemographicRecord) {
          let course_learner_id = processNull(learner["course_learner_id"]),
            ...
            year_of_birth = parseInt(processNull(learner["year_of_birth"])),
 ...
 * @returns {{}[]} Array with objects of demographic records
 */
export function processDemographics(
  ...
  return new Promise((resolve, reject) => {
    $.getJSON(inputFile, function (learnerDemographicData) {
      let learnerDemographicRecord = [];
      for (let learner of learnerDemographicData) {
        let course_learner_id = learner["hash_id"],
          gender = learner["gender"],
      ...
      resolve(learnerDemographicRecord);
    }).fail(function (jqXHR, textStatus, error) {
      reject(
        new Error(`Failed to load demographic data: ${textStatus} (${error})`),
      );
    });
  });
}
