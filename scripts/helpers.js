...
export function verifyBrowser() {
  ...
  if (!["Chrome", "Firefox"].includes(userBrowser.browser.name)) {
    toastr.error(
      "ELAT is developed mainly for Chrome and supports Firefox, please try one of those browsers",
 ...
 */
export function learnerSegmentation(learnerId, segmentation, connection) {
  if (!segmentation) {
...
export function downloadForumSegmentation(connection) {
  connection
    .runSql("SELECT * FROM webdata WHERE name = 'studentsForumBehavior' ")
    .then(function (result) {
      if (result.length === 0) {
        loader(false);
        toastr.error(
          "Graph data has to be processed again, click the Update Graph Values button",
        );
      } else {
