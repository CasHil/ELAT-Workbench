 ...
 */
export function processGeneralSessions(
  courseMetadataMap,
  logFiles,
  fileIndex,
  totalFiles,
  ...
  for (let f in logFiles) {
    ...
    if (file_name.includes("log")) {
      ...
      learner_all_event_logs = JSON.parse(
        JSON.stringify(updated_learner_all_event_logs),
      );
      ...
      console.log("Starting general session processing");
 ...
 * @param {JsStoreWorker} connection Main JsStore worker that handles the connection to SqlWeb
 */
export function processForumSessions(
  courseMetadataMap,
  logFiles,
  index,
  total,
  ...
  for (let log_file of logFiles) {
    ...
    if (file_name.includes(current_date) || true) {
      ...
      for (let line of lines) {
        ...
        let event_type = jsonObject["event_type"];
        if (
          event_type.includes("/discussion/") ||
          forum_event_types.includes(event_type)
        ) {
        ...
        for (let i in event_logs) {
          ...
          if (session_id === "") {
            ...
            if (
              ...
              if (
                new Date(event_logs[i]["event_time"]) >
                verification_time.setMinutes(
                  verification_time.getMinutes() + 30,
                )
              ) {
                session_id =
                  session_id +
  ...
  if (forum_sessions_record.length === 0) {
    ...
    for (let array of forum_sessions_record) {
      ...
      let values = {
        ...
        end_time: end_time,
        duration: duration,
        relevent_element_id: rel_element_id,
      };
      data.push(values);
    }
    if (connection === null) {
 ...
 * @param {JsStoreWorker} connection Main JsStore worker that handles the connection to SqlWeb
 */
export function processVideoInteractionSessions(
  courseMetadataMap,
  logFiles,
  index,
  total,
      ...
      for (let line of lines) {
        ...
        if (video_event_types.includes(jsonObject["event_type"])) {
          ...
          if (global_learner_id !== "") {
            ...
            if (
              ["seek_video", "edx.video.position.changed"].includes(event_type)
            ) {
            ...
            if (
              ...
              if (
                ...
                if (speed_change_last_time === "") {
                  ...
                  if (
                    (log["event_time"] - speed_change_last_time) / 1000 >
                    10
                  ) {
                    let old_speed = log["old_speed"];
                    let new_speed = log["new_speed"];
              ...
              if (
                ...
                if (
                  ["pause_video", "edx.video.paused"].includes(
                    log["event_type"],
                  )
                ) {
                  pause_check = true;
  ...
  for (let interaction_id in video_interaction_map) {
    const video_interaction_id = interaction_id,
      course_learner_id =
        video_interaction_map[interaction_id]["course_learner_id"],
 ...
 * @param {JsStoreWorker} connection Main JsStore worker that handles the connection to SqlWeb
 */
export function processAssessmentsSubmissions(
  courseMetadataMap,
  logFiles,
  index,
  total,
 ...
 * @param {JsStoreWorker} connection Main JsStore worker that handles the connection to SqlWeb
 */
export function processQuizSessions(
  courseMetadataMap,
  logFiles,
  index,
  total,
 ...
 */
export function processORASessions(
  courseMetadataMap,
  logFiles,
  index,
  total,
 ...
 * @param {function} callback Callback to prepareLogFiles function, providing the file index and chunk index to continue
 */
export function findORASessions(
  courseMetadataMap,
  logFiles,
  index,
  total,
